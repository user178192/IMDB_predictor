...
void LanguagesParser::ProcessPart1(string part1)
{
...
void LanguagesParser::parseLine(const std::string input_line) {
    ...
    for (int i = 0; i < input_len; ++i) {
        if (input_line[i] == '\t') {
            ...
            while(input_line[i] == '\t')
            	++i;
            lang_ = input_line.substr(i,input_len - i);
            i = input_len;
        }                            
    }//end of for
    ...
    //cout<<title_<<" "<<subtitle_<<" "<<type_<<" ["<<lang_<<"]\n";
    ...
    if (get<0>(db_ret)) {
        ...
        if (!lang_.empty())
        	get<2>(db_ret)->languages_.push_back(lang_);
    } else {
